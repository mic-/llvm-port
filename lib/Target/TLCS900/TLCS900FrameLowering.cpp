...
void TLCS900FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  const TLCS900InstrInfo &TII =
      *static_cast<const TLCS900InstrInfo *>(MF.getSubtarget().getInstrInfo());
  ...
  if (hasFP(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(TLCS900::PUSH16r))
      .addReg(TLCS900::FP, RegState::Kill);

    // Update FP with the new base value...
    BuildMI(MBB, MBBI, DL, TII.get(TLCS900::MOV16rr), TLCS900::FP)
      .addReg(TLCS900::SP);

    // Mark the FramePtr as live-in in every block except the entry.
    for (MachineFunction::iterator I = std::next(MF.begin()), E = MF.end();
         I != E; ++I)
      I->addLiveIn(TLCS900::FP);
  ...
  if (NumBytes) { // adjust stack pointer: SP -= numbytes
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(TLCS900::SUB16ri), TLCS900::SP)
        .addReg(TLCS900::SP).addImm(NumBytes);
...
void TLCS900FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    ...
    // pop FP.
    BuildMI(MBB, MBBI, DL, TII.get(TLCS900::POP16r), TLCS900::FP);
  } else
  ...
  if (MFI->hasVarSizedObjects()) {
    BuildMI(MBB, MBBI, DL,
            TII.get(TLCS900::MOV16rr), TLCS900::SP).addReg(TLCS900::FP);
    if (CSSize) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL,
                TII.get(TLCS900::SUB16ri), TLCS900::SP)
        .addReg(TLCS900::SP).addImm(CSSize);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
  } else {
    ...
    if (NumBytes) {
      MachineInstr *MI =
        BuildMI(MBB, MBBI, DL, TII.get(TLCS900::ADD16ri), TLCS900::SP)
        .addReg(TLCS900::SP).addImm(NumBytes);
      // The SRW implicit def is dead.
      MI->getOperand(3).setIsDead();
    }
    ...
    if (Amount != 0) {
      ...
      if (Old->getOpcode() == TII.getCallFrameSetupOpcode()) {
        New = BuildMI(MF, Old->getDebugLoc(),
                      TII.get(TLCS900::SUB16ri), TLCS900::SP)
          .addReg(TLCS900::SP).addImm(Amount);
      } else {
        ...
        if (Amount)
          New = BuildMI(MF, Old->getDebugLoc(),
                        TII.get(TLCS900::ADD16ri), TLCS900::SP)
            .addReg(TLCS900::SP).addImm(Amount);
      }
    ...
    if (uint64_t CalleeAmt = I->getOperand(1).getImm()) {
      ...
      MachineInstr *New =
        BuildMI(MF, Old->getDebugLoc(), TII.get(TLCS900::SUB16ri),
                TLCS900::SP).addReg(TLCS900::SP).addImm(CalleeAmt);
      // The SRW implicit def is dead.
      New->getOperand(3).setIsDead();
...
void
TLCS900FrameLowering::processFunctionBeforeFrameFinalized(MachineFunction &MF,
                                                         RegScavenger *) const {
