...
TLCS900TargetLowering::TLCS900TargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i8,  &TLCS900::GR8RegClass);
  addRegisterClass(MVT::i16, &TLCS900::GR16RegClass);
  ...
  setStackPointerRegisterToSaveRestore(TLCS900::SP);
...
TLCS900TargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':   // GENERAL_REGS
      if (VT == MVT::i8)
        return std::make_pair(0U, &TLCS900::GR8RegClass);

      return std::make_pair(0U, &TLCS900::GR16RegClass);
    }
  }
...
static void AnalyzeVarArgs(CCState &State,
  ...
  State.AnalyzeCallOperands(Outs, CC_TLCS900_AssignStack);
}

static void AnalyzeVarArgs(CCState &State,
                           const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeFormalArguments(Ins, CC_TLCS900_AssignStack);
}
...
static void AnalyzeArguments(CCState &State,
  ...
  static const MCPhysReg RegList[] = {
    TLCS900::R15, TLCS900::R14, TLCS900::R13, TLCS900::R12
  };
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UseStack && Parts <= RegsLeft) {
      ...
      UseStack = true;
      for (unsigned j = 0; j < Parts; j++)
        CC_TLCS900_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
    }
  }
}
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_TLCS900);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_TLCS900);
}
...
SDValue
TLCS900TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
...
TLCS900TargetLowering::LowerCCCArguments(SDValue Chain,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i16:
        unsigned VReg = RegInfo.createVirtualRegister(&TLCS900::GR16RegClass);
...
TLCS900TargetLowering::LowerCCCCallTo(SDValue Chain, SDValue Callee,
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, dl, TLCS900::SP, PtrVT);
...
SDValue TLCS900TargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(TLCS900ISD::Wrapper, SDLoc(Op), PtrVT, Result);
}
...
SDValue TLCS900TargetLowering::LowerExternalSymbol(SDValue Op,
  ...
  return DAG.getNode(TLCS900ISD::Wrapper, dl, PtrVT, Result);
}
...
SDValue TLCS900TargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    SDValue SR = DAG.getCopyFromReg(DAG.getEntryNode(), dl, TLCS900::SR,
                                    MVT::i16, Flag);
    if (Shift)
    ...
    return DAG.getNode(TLCS900ISD::SELECT_CC, dl, VTs, Ops);
  }
}
...
SDValue TLCS900TargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  TLCS900MachineFunctionInfo *FuncInfo = MF.getInfo<TLCS900MachineFunctionInfo>();
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue TLCS900TargetLowering::LowerJumpTable(SDValue Op,
                                             SelectionDAG &DAG) const {
    JumpTableSDNode *JT = cast<JumpTableSDNode>(Op);
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Result = DAG.getTargetJumpTable(JT->getIndex(), PtrVT);
    return DAG.getNode(TLCS900ISD::Wrapper, SDLoc(JT), PtrVT, Result);
}
...
const char *TLCS900TargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((TLCS900ISD::NodeType)Opcode) {
  case TLCS900ISD::FIRST_NUMBER:       break;
  ...
  case TLCS900ISD::SETCC:              return "TLCS900ISD::SETCC";
  ...
  case TLCS900ISD::SRL:                return "TLCS900ISD::SRL";
  }
  return nullptr;
}
...
bool TLCS900TargetLowering::isZExtFree(SDValue Val, EVT VT2) const {
...
TLCS900TargetLowering::EmitShiftInstr(MachineInstr *MI,
  ...
  case TLCS900::Shl8:
   Opc = TLCS900::SHL8r1;
   RC = &TLCS900::GR8RegClass;
   break;
  case TLCS900::Shl16:
   Opc = TLCS900::SHL16r1;
   RC = &TLCS900::GR16RegClass;
   break;
  case TLCS900::Sra8:
   Opc = TLCS900::SAR8r1;
   RC = &TLCS900::GR8RegClass;
  ...
  unsigned ShiftAmtReg = RI.createVirtualRegister(&TLCS900::GR8RegClass);
  unsigned ShiftAmtReg2 = RI.createVirtualRegister(&TLCS900::GR8RegClass);
